...
const BlockTable = ({ ProfilePop, updateProfilePop }) => {
  const dispatch = useDispatch();

  const { blocked_user } = useSelector((s) => s.user) ?? {};
  const handleProfilePop = (id, e) =>
    updateProfilePop({
      id,
      profilePop: !ProfilePop.profilepop,
    });

  const data = blocked_user?.data;

  const handleBlocking = async (id, e) => {
    try {
      let request = {
        id,
      };
      let y = await dispatch(blockUnblockUser(request)).unwrap();
    } catch (error) {
    ...
    <>
      <div className="table-responsive">
        <table className="table commonTable">
          <thead>
            <tr>
              <th className="p-3 border-0">S.No.</th>
              <th className="p-3 border-0">Name</th>
              <th className="p-3 border-0">Mobile number</th>
              <th className="p-3 border-0">Email Address</th>
              <th className="p-3 border-0">Registered at</th>
              <th className="p-3 border-0">Action</th>
            </tr>
          </thead>
          <tbody>
            {data &&
                ...
                return (
                  <>
                    <tr>
                      ...
                      <td className="p-3 border-0">{data.createdAt}</td>
